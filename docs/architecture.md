...
5. 使用同一文件系统的 `link()` 将完整临时文件原子发布到公开根目录。目标已存在时直接失败，包括同名文件、目录和符号链接；并发上传也不会覆盖。
6. 成功或失败均移除该次上传的临时目录，不修改已有同名文件。清理失败记录错误日志；进程被强制终止时可能留下私有临时文件，但不会公开残缺文件。
...
- `DATA_DIR`：已存在的数据根目录。
- `UPLOAD_TMP_DIR`：私有上传临时目录，默认 `<DATA_DIR 的真实路径>.tmp`；不能与公开目录重叠，且必须在同一文件系统。部署脚本配置为 `/var/lib/sharebox/tmp`，并加入 systemd 可写路径。
