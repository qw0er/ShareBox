...
| `app/server/tus.server.ts` | tus FileStore、来源/大小校验、共享锁、复制发布、回执及过期清理 |
...
页面数据由 loader 提供；上传通过 tus 资源路由，发布成功后主动重新验证 loader；删除公开文件通过 fetcher 调用 action。旧 multipart 上传服务保留兼容。展示组件不直接访问文件系统，上传组件也不依赖文件树的展示组件。`app/test/tus.server.test.ts` 覆盖续传、重启、中断、删除、冲突、过期，`app/test/upload-queue.test.ts` 覆盖客户端进度和任务控制。
